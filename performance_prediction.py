...
print(len(sw_df))
print("--- MODEL PERFORMANCE RAPORT ---")
print(f"Success Rate (R2 Score): %{r2_score(y_test, y_pred)*100:.2f}")
print(f"Error (MSE): {mean_squared_error(y_test, y_pred):.4f}")
print("-" * 30)

plt.figure(figsize=(8, 6))
plt.scatter(y_test, y_pred, alpha=0.6, color='darkgreen', label='Guesses')
